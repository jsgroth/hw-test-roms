...
/* Interrupt vectors */
.align 2
v_hint_vector: dc.l _noop_int_handler
v_vint_vector: dc.l _noop_int_handler
...
RomHeader:
        .ascii	"SEGA GENESIS    "		/* First 4 bytes must be "SEGA" */
        .ascii	"GRIND   2019.DEC"		/* Copyright and date */
        .ascii	"VDP Basic Tests                                 " /* JP Name */
        .ascii	"VDP Basic Tests                                 " /* EN Name */
        ...
        .ascii	"  "					/* "RA" to enable SRAM, "  " to disable */
...
VBlank:
        movem.l d0-d1/a0-a1, -(sp)
        move.l v_vint_vector, a0
        jsr (a0)
        movem.l (sp)+, d0-d1/a0-a1
        rte

/* Called by HBlank VSRAM update test */
/* Performs a VDP data port as soon as possible after V counter reaches the target line */
/* Expects line (b) in D0 and data port value (w) in D1 */
/* Control port should already be initialized */
.globl hblank_vsram_update
hblank_vsram_update:
        movem.l a0-a1, -(sp)

        move.l #0xC00000, a0
        move.l #0xC00008, a1
hblank_vsram_update_loop:
        cmp.b (a1), d0
        bne hblank_vsram_update_loop
